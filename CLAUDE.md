...
4. **Offer to start the first job right away.** For `social-video-producer`, ask whether they want a full script-to-video production or an assembly from existing clips/audio. For assembly, offer to fill `social-video-producer/PROMPT-template.md` together: ask for each placeholder value conversationally, show the filled prompt, and ask "Do I apply this prompt and start?" before proceeding.
