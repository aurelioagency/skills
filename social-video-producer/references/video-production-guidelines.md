...
- Check the face position in **every camera framing the video uses**, not one frame. When the framing changes and the chin moves vertically, the caption band must move with it — never one fixed position for the whole video. For burned-in captions that is `--zones` on `build-burn-in-captions.mjs`. "Below the face" also means not stranded at the very bottom in the shots where the face rides high.
...
- **The caption band tracks the face across shots. A single fixed `--margin-bottom` is only right for a video that holds one framing the whole way through.** Talking-head footage rarely does: a wide intro cuts to a close-up, the speaker leans in, a background screen-recording appears and the face drops. A band that clears the chin in the intro then sits across the mouth. Before choosing positions, scan the whole clip, identify each framing, and find the LOWEST the chin gets in each. Then pass `--zones` — a JSON array of `{ start, end, marginBottom }` windows — so each caption sits just below its own shot's chin (and not stranded at the frame bottom in the shots where the face is high). Keep the zones file in the project so it can be re-tuned and reburned in seconds.
- A chunk that wraps to two lines grows *upward* from the baseline, so a 2-line caption can touch the chin in a low-face shot even when the 1-line captions clear it. The width gate cannot see this. Give that chunk's moment its own lower micro-zone, or size the type down until nothing wraps.
