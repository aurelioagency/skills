...
| [business-ai-audit](business-ai-audit/) | Evidence-led business workflow discovery that produces a living roadmap of practical AI, automation, and process-improvement opportunities. |
...
| [hook-generator](hook-generator/) | Hooks for any content, from a topic in any format (text, image, PDF, link, transcript): one hook written per each of 10 psychological structures, plus an assessment of which ones actually fit that topic — where the number of recommendations is a result, not a quota. Text only, no dependencies. Answers in the language you ask in; written and calibrated for Rioplatense Spanish. |
| [social-video-producer](social-video-producer/) | Short vertical video production (TikTok / Reels / Shorts, 1080×1920), segment-first so changing one hook never re-renders the rest: script to video with ElevenLabs TTS and HeyGen avatar openings/outro over a HyperFrames animated body, assembly from footage you already filmed, burned-in word-level subtitles for a finished video, licensed background music, approval gates, paid-provider protection, encode budget, and QA/verification gates. |
| [kieai-minimax-h3](kieai-minimax-h3/) | Short visual-effect clips (4–15 s) with MiniMax H3 on KIE.ai: technique selection from a bundled 136-entry [eyecannndy](https://eyecannndy.com) catalogue, the prompt clauses this model actually needs (framing, scale, anchors), image-to-video / reference-to-video / text-to-video, a credit estimate and confirmation gate before anything is submitted, polling and download, and automatic verification with an ffmpeg contact sheet against a six-point checklist. |
...
| [ai-automation-contracts](ai-automation-contracts/) | Ready-to-sign contracts, worded in the provider's favor, for selling AI automation projects: full turnkey implementation, an optional monthly maintenance retainer, and one-page work orders for return visits. Fixed policy on IP split, liability cap, tacit acceptance and a three-layer AI performance standard, an intake checklist that only asks for what's missing, and local `.docx` generation with a PDF check before delivery — no API keys. |
...
For example, for [social-video-producer](social-video-producer/README.md):
...
The generic pattern for any skill in the table above is the same with `<skill-name>` swapped in. Installing one skill never touches other skills you already have. Each skill's prompt template (e.g. [PROMPT-template.md](social-video-producer/PROMPT-template.md)) also self-installs its skill if it is missing, so you can simply fill in the placeholders and paste it as your first message.
