...
1. **Bajar el audio con cobalt.tools.** La API pública de cobalt tiene bot-protection y rechaza llamadas directas (ver `references/cobalt.md`), así que se usa la página web real: `cobalt.tools`, pegar el link del reel, elegir modo audio, descargar. Usá `Claude in Chrome` (el navegador real del usuario) para esto, no el browser sandboxeado — así el archivo cae en la carpeta de Descargas real y se puede mover con Bash/PowerShell. Movelo a `aprobados\<reel-id>\audio.<ext>`. **Si el archivo que baja es una imagen (~100KB, `.jpg`) en vez de audio/video, cobalt no pudo pasar el bloqueo de Instagram para ese reel** — no reintentar en loop, caer directo a `yt-dlp` como respaldo (ver el final de `references/cobalt.md`).
...
4. **Recurso mencionado.** El recurso (app, sitio, producto, link) casi siempre se dice en el propio audio, no en la descripción — buscalo en el diálogo transcripto. Si no aparece ahí, revisá la descripción del reel. Si el reel no muestra el link (solo dice "comentá X y te lo mando"), buscá vos el nombre del recurso (ej. en GitHub o la web) para conseguir el link real en vez de dejarlo colgado del funnel del creador. Guardá lo que encuentres (nombre + link si lo tiene) en `aprobados\<reel-id>\recurso.txt`, o escribí "no se menciona ningún recurso" si no hay nada.
5. **Si es un repo de GitHub, encontrá el original antes de desconfiar.** Que aparezcan varios repos con el mismo nombre bajo cuentas distintas **no es de por sí sospechoso** — en GitHub cualquiera puede hacer fork de un repo público, y eso se ve clarísimo en la propia página: dice "forked from `<original>`" arriba del nombre, y muestra cuántos commits está atrasado respecto al original. Antes de calificar algo de trucho, entrá a los repos y fijate esa etiqueta — casi siempre hay un original real (con más issues, PRs y commits que el resto) y el resto son forks normales de gente que lo usa o contribuye. Recién si un repo NO tiene relación de fork con los demás (son independientes, sin ese aviso) y aun así comparten nombre y descripción calcada, ahí sí es señal de clon/spam — avisale al usuario en ese caso puntual.
